      ...
      <Card className="p-5 md:p-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <FormField error={errors.title?.message} id="title" label="Package title / description" required>
            <Input disabled={isLoading} id="title" placeholder="Medical supplies — priority carton" {...register("title")} />
          </FormField>
          <FormField description="Optional" error={errors.estimatedDeliveryDate?.message} id="estimatedDeliveryDate" label="Estimated delivery date">
            <Input disabled={isLoading} id="estimatedDeliveryDate" type="date" {...register("estimatedDeliveryDate")} />
          </FormField>
        </div>
      </Card>
      ...
      <Card className="p-5 md:p-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <FormField error={errors.senderName?.message} id="senderName" label="Sender name" required>
            <Input disabled={isLoading} id="senderName" placeholder="Central Warehouse" {...register("senderName")} />
          </FormField>
          <FormField description="Optional" error={errors.senderContact?.message} id="senderContact" label="Sender phone or email">
            <Input disabled={isLoading} id="senderContact" placeholder="+1 555 0100" {...register("senderContact")} />
          </FormField>
          <FormField error={errors.receiverName?.message} id="receiverName" label="Receiver name" required>
            <Input disabled={isLoading} id="receiverName" placeholder="Regional Distribution Center" {...register("receiverName")} />
          </FormField>
          <FormField description="Optional" error={errors.receiverContact?.message} id="receiverContact" label="Receiver phone or email">
            <Input disabled={isLoading} id="receiverContact" placeholder="+1 555 0199" {...register("receiverContact")} />
          </FormField>
        </div>
      </Card>
      ...
      <Card className="p-5 md:p-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <FormField error={errors.origin?.message} id="origin" label="Origin" required>
            <Input disabled={isLoading} id="origin" placeholder="Houston, TX" {...register("origin")} />
          </FormField>
          <FormField error={errors.destination?.message} id="destination" label="Destination" required>
            <Input disabled={isLoading} id="destination" placeholder="Atlanta, GA" {...register("destination")} />
          </FormField>
          <FormField error={errors.currentLocation?.message} id="currentLocation" label="Current location" required>
            <Input disabled={isLoading} id="currentLocation" placeholder="Dallas, TX hub" {...register("currentLocation")} />
          </FormField>
          <FormField error={errors.currentStatus?.message} id="currentStatus" label="Current status" required>
            <Select disabled={isLoading} id="currentStatus" {...register("currentStatus")}>
